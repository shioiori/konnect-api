...
namespace UTCClassSupport.API.Utilities
{
  public static class AesOperation
  {
    ...
    private const int BASE36_BLOC_SIZE_36 = 13; 
    ...
    public static string GetEncryptText(string token)
    {
      return token.Remove(0, 32);
    }

    public static string GenerateToken(string key, string encryptText)
    {
      return key + encryptText;
    }

    public static string GenerateKey()
    {
      var key = Guid.NewGuid().ToString();
      return key.Replace("", string.Empty).ToUpper();
    }
    ...
    public static string EncryptString(string key, string plainText)
    {
      ...
      return ByteArrayToBase36String(array);
    }

    public static string DecryptString(string key, string cipherText)
    {
      byte[] iv = new byte[16];
      byte[] buffer = Base36StringToByteArray(cipherText);
